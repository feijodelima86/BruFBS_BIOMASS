...
a<-data.frame(aggregate(as.numeric(alldata[,n1]) ~ DATE+SITE, alldata, mean))
a[,4]<-aggregate(as.numeric(alldata[,n1]) ~ DATE+SITE, alldata, se)[,3]
...
a2<-data.frame(aggregate(as.numeric(alldata[,n2])+as.numeric(alldata[,n1]) ~ DATE+SITE, alldata, mean))
a2[,4]<-aggregate(as.numeric(alldata[,n2])+as.numeric(alldata[,n1]) ~ DATE+SITE, alldata, se)[,3]
...
plot(a[,1], a[,2],
     type='n',
     ylim=c(12,round_any(max(a2$MEAN+a2$STDER, na.rm=T), 100, f = ceiling)),
     xlim= c(min(a$DATE),max(a$DATE)),
     ylab=NA, 
     yaxt = "n",
     cex=1.5,
     xaxt = "n",
...
yrange<-round_any(max(a2$MEAN+a2$STDER, na.rm=T), 100, f = ceiling)

#axis(side = 2, at = c(0, 25, 50, 75, 100), las=1, font.axis=1, cex.axis=1.75)
axis(side = 2, at = seq(0, yrange, length.out=5), las=1, font.axis=1, cex.axis=1.75)
